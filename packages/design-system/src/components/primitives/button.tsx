...
export function DesignSystemButton({
  ...
  return (
    <button
      className={cn(
        "font-body rounded-token-xs inline-flex shrink-0 items-center justify-center gap-[10px] border text-[14px] font-[700] leading-[20px] transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#121318] focus-visible:ring-offset-2",
        size === "large"
          ? "px-token-24 h-[52px] w-[354px]"
          : "px-token-12 h-[36px] w-[100px]",
    ...
    >
      {showIcon ? (
        <span
          className={cn(
            "size-token-16 rounded-token-xxs flex items-center justify-center",
            state === "disabled"
              ? "bg-[#8d8e91] text-[#f4f4f4]"
              : "bg-[#121318] text-white",
          )}
        >
          <X className="size-token-12" strokeWidth={3} />
        </span>
      ) : null}
      <span>{children}</span>
    </button>
