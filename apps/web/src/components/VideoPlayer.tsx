...
export function VideoPlayer() {
  ...
  const navigateToPrevious = useCallback(() => {
    ...
    setIsAnimating(true);
    setDirection(-1);
    setCurrentVideoIndex((index) => getPrevIndex(index));
  }, [isAnimating]);
  ...
  const handleTouchEnd = useCallback(
    (event: React.TouchEvent<HTMLDivElement>) => {
      if (touchStartYRef.current === null || isAnimating) return;
  ...
  return (
    ...
    >
      ...
      <div
        ref={containerRef}
        className="relative aspect-[9/16] h-[min(78svh,800px)] min-h-[520px] w-full max-w-[420px] overflow-visible"
      >
