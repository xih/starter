...
>(({ className, ...props }, ref) => {
  ...
  return (
    <button
      ...
      className={cn(
        "absolute inset-y-0 z-20 hidden w-4 -translate-x-1/2 transition-all ease-linear after:absolute after:inset-y-0 after:left-1/2 after:w-token-2 hover:after:bg-sidebar-border group-data-[side=left]:-right-4 group-data-[side=right]:left-0 sm:flex",
        className,
      )}
      {...props}
    />
...
>(({ className, ...props }, ref) => (
  <input
    ...
    data-sidebar="input"
    className={cn(
      "flex h-9 w-full rounded-md border border-sidebar-border bg-background px-3 py-1 text-sm shadow-sm outline-none placeholder:text-muted-foreground focus-visible:ring-1 focus-visible:ring-sidebar-ring",
      className,
    )}
    {...props}
  />
...
>(({ className, asChild = false, ...props }, ref) => {
  ...
  return (
    <Comp
      ...
      className={cn(
        "text-sidebar-foreground/70 flex h-8 shrink-0 items-center rounded-md px-2 text-xs font-medium uppercase tracking-normal outline-none transition-[margin,opacity] duration-200 ease-linear group-data-[collapsible=icon]:-mt-8 group-data-[collapsible=icon]:opacity-0",
        className,
      )}
      {...props}
    />
