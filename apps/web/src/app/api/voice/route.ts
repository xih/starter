...
export async function POST(request: Request) {
  ...
  if (!groq) {
    return new Response("GROQ_API_KEY is required", { status: 500 });
  }
  ...
  const transcript = await getTranscript(groq, data.input);
...
async function getTranscript(groq: Groq, input: string | File) {
