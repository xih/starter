...
`LIVEKIT_AGENT_PROVIDER` defaults to `livekit`, which uses LiveKit hosted STT,
LLM, and TTS inference and consumes LiveKit inference gateway credits. Set it
to `openai` only when intentionally using direct OpenAI STT, LLM, and TTS with
`OPENAI_API_KEY`.
